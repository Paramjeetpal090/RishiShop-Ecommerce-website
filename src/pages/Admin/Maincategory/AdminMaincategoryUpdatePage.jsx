...
export default function AdminMaincategoryUpdatePage() {
  ...
  let [MaincategoryStatePage, setMaincategoryStateData] = useState([])
  ...
  function postData(e) {
    ...
    if (error)
      ...
      let item = MaincategoryStatePage.find(x => x.id !== id && x.name?.toLowerCase() === data.name?.toLowerCase())
      if (item) {
  ...
  useEffect(() => {
    (() => {
      ...
      if(setMaincategoryStateData.length){
    ...
    <div className="container">
      <div className="row">
        ...
        <div className="col-lg-9">
          <h5 className='bg-secondary text-center p-2 text-light'>Update Maincategory <Link to="/admin/maincategory"><i className='bi bi-arrow-left text-light fs-5 float-end'></i></Link></h5>
          <form onSubmit={postData}>
            <div className="row">
              <div className="col-12 mb-3">
                <label> Name*</label>
                <input type="text" name='name' value={data.name} onChange={getInputData} placeholder='Maincategory Name' className={`form-control ${show && errorMessage.name ? `border-danger` : `border-secondary`}`} />
