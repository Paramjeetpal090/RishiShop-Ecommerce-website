...
export default function AdminSubcategoryCreatePage() {
  ...
  let SubcategoryStatePage=useSelector(state=>state.setSubcategoryStateData)
...
setErrorMessage({...errorMessage,[name]:name==="pic"?ImageValidator(e) :FormValidator(e)})
 ...
 function postData(e){
    ...
    if(error)
      ...
      let item = SubcategoryStatePage.find(x=> x.name.toLowerCase()===data.name?.toLowerCase())
      if(item){
  ...
  },[setSubcategoryStateData.length])
  return (
   <div className="container">
    <div className="row">
      ...
      <div className="col-lg-9">
        <h5 className='bg-secondary text-center p-2 text-light'>Create subcategory <Link to="/admin/subcategory"><i className='bi bi-arrow-left text-light fs-5 float-end'></i></Link></h5>
        <form onSubmit={postData}>
          <div className="row">
            <div className="col-12 mb-3">
             <label> Name*</label>
             <input type="text" name='name' onChange={getInputData} placeholder='subcategory Name' className={`form-control ${show && errorMessage.name?`border-danger`:`border-secondary`}`}/>
